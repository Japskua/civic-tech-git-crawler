...
The community-forum platform behind LessWrong and the Effective Altruism Forum (TypeScript, 2012). **Largest commit count in the sample (52,222) and largest net-positive LOC delta (+7M)**. Bus factor 4, HHI 1,059 — **the lowest HHI / most distributed core in the dataset** despite individual `jimrandomh` accounting for 24% of all lines. PR turnaround 0.2 h is exceptional; 264 attributable developers. **Summary**: the example of how a long-lived, modestly-popular project can build a genuinely distributed core team — the opposite tail of AutoGPT's profile.

## Significant-Gravitas/AutoGPT

Highest-stars project in the sample (184k★, 46k forks). 430 attributable developers but **bus factor 1**, HHI 4,448, **lines-Gini 0.99** — extreme effort concentration despite massive contributor breadth. Top contributor `waynehamadi` does 23% of lines; PR turnaround 0.1 h. Issue first response 22 h, 3,815 issues, 70% closed. Health % is 100. **Summary**: textbook "drive-by celebrity project" — thousands of contributors, but a very small team actually shipping merged code; the dataset's strongest example of star count not predicting contributor distribution.
...
The Volunteer Income Tax Assistance backend (2019, 50 developers, 1.5M LOC added). Bus factor 4 (one of the most resilient cores in codeforamerica). **Lowest single-contributor week percentage in the new sample (3%)** alongside civiform and AutoGPT — these three are the only repos in the dataset with genuinely distributed weekly contribution. **Summary**: the codeforamerica project that most resembles a sustainable open-source community.
...
**Bus-factor 1 is the modal outcome** — 16 of 38 repositories (42%) have `bus_factor_no_bots = 1`, meaning a single contributor accounts for ≥50% of the project's commits. The rate is similar in both cohorts (new 9: 5/9, original 29: 11/29) so this isn't a sample-composition artefact.

**Effort concentration scales sub-linearly with project size**. The four most-collaborative weekly profiles (mean top-share <60%) come from civiform, vita-min, AutoGPT, and meshtastic/firmware — projects with 90+ developers and active corporate or community sponsorship. Below that scale threshold, single-author weeks are the norm regardless of organisation.
