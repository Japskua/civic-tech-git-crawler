...
| **person_metrics rows** | 702 (653 human, 49 bot) |
| **contributor_weekly_activity unique contributors** | 2,340 |
| **contributor-weeks** | 22,333 |
| **Total commits (`repo_metrics`)** | 178,099 |
| **Total commits (CWA)** | 160,959 |
| **Lines added (cumulative)** | 42,400,713 |
| **Lines removed (cumulative)** | 33,001,118 |
| **PR records** | 77,780 |
...
| Burstiness coverage | 5/38 (most repos NaN due to GitHub stats endpoint timeouts) | **36/37** (recomputed from `weekly_snapshots`) |
...
| `chaoss_summary.csv` | 37 | 45+ CHAOSS columns including `burstiness_cv` (recomputed from weekly_snapshots, 36/37 populated) and the new `burstiness_cv_full_history` column |
...
| `weekly_snapshots.csv` | 5,664 | Weekly commit/contributor snapshots |
| `contributor_lifecycles.csv` | 2,340 | Per-contributor first/last commit, active/departed |
| `contributor_weekly_activity.csv` | 22,333 | Per-(contributor, ISO-week) commit + lines_added + lines_removed |
