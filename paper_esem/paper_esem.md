...
**Novel contributions of this paper:** (3) *effort concentration systematically exceeds activity concentration* — effort-weighted Gini exceeds commit-count Gini in 27 of 37 repositories (Wilcoxon W = 53, p = 4.8 × 10⁻⁵, rank-biserial r = +0.81, large effect); we introduce the *elephant-week* metric, with a per-repository median 96.6% of active weeks dominated by a single contributor; and (6) the *cross-project ecosystem is umbrella-bounded* — 5.5% of unique humans in our sample (112 of 2,055) are active in ≥ 2 repositories in our sample, but only 8.9% of *those* (10 of 112) span ≥ 2 stewarding organisations, leaving ≈ 0.5% of all humans in our sample genuinely cross-organisational.
...
1. **Effort concentration is systematically worse than commit counts suggest.** Most OSS-health frameworks (e.g., CHAOSS [8]) measure contribution distribution by counting commits per contributor. We compare two measures of contribution inequality across the same 37 repositories: *commit-Gini* (commits per contributor) and *line-Gini* (lines added + removed per contributor). The line-Gini exceeds the commit-Gini in 27 of 37 repositories (paired Wilcoxon signed-rank, p = 4.8 × 10⁻⁵, rank-biserial r = +0.81, large effect). The implication is that a civic-tech project whose commit counts look balanced across contributors can in fact have most of its real work concentrated in one person making large refactors while others contribute small fixes. As a per-week extension we introduce the **elephant-week** metric (§3.3): a repository-week is an *elephant week* if a single contributor accounts for ≥ 50% of that week's added + removed lines. The median repository in our sample has elephant weeks in 96.6% of its active weeks — almost every active week of the median civic-tech project is dominated by one contributor.

2. **Cross-project contribution is bounded by umbrella organisations, not by topic.** A common implicit picture is that civic-tech is one community with contributors flowing between projects on shared values. The data disagrees. Of 2,055 unique humans contributing across the 37 repositories, only 5.5% (112) contribute to more than one of them — and of those 112 cross-project humans, only 8.9% (10) span more than one stewarding organisation, leaving ≈ 0.5% of all humans in our sample as genuinely cross-organisational contributors. Because the 0.5% figure measures cross-*organisational* mobility, it is invariant to how heavily we sampled from umbrella networks in our frame, so the pattern survives a sensitivity check on sampling-frame bias (§6). Contributor-wise, civic-tech is closer to a set of mostly-isolated umbrella ecosystems than a single field-spanning community, with direct implications for sustainability programmes that assume cross-cutting volunteer flow.

**Confirming established small-team-OSS patterns at landscape scale in civic-tech.** Two patterns previously reported for general open-source hold in our 37-repository sample: drive-by contributor dominance [10] — 52% of human contributor records make a single commit and never return — and low truck-factor distributions [1] — median bus factor 2, with 46% of repositories at bus factor 1 (a single developer accounts for ≥ 50% of commits). We report these to characterise the civic-tech landscape and to mark which of our findings extend prior work versus break new ground.

**Supporting infrastructure.** An operational definition of civic technology with three binary inclusion criteria applied independently by two coders (§3.1); an open-source Python toolchain; the canonical May 2026 dataset (37 repositories, 22,486 contributor-weeks); and deterministic figure-regeneration scripts.
...
- **RQ3.** What community-health gaps (stale issues, missing documentation, missing release discipline) are visible across the 37 repositories, and how widely?
...
**OSS health and contributor dynamics.** The CHAOSS framework [8] standardises OSS community-health metrics including the bus factor [1] and the HHI, both flagged as sustainability risks across general-purpose OSS; recent work in this tradition has shown how OSS communities use these metrics for self-comparison and social calibration [17]. Coelho and Valente [3] identified contributor departure as the primary cause of unmaintained projects on GitHub. Pinto et al. [10] characterised casual contributors at scale and argued that their cumulative contribution is significant despite individual brevity. Eghbal [4] characterised maintainer-volunteer dynamics in the long tail of OSS. DORA-style delivery indicators [5, 6] were designed for commercial teams and have not been systematically characterised for civic-tech. Dey et al. [2] and Golzadeh et al. [7] proposed heuristic and supervised methods for bot identification; bot filtering matters because automated accounts inflate organisational concentration metrics without sustaining the project.

**Civic technology.** Civic-tech adoption and investment surveys [11, 9, 16] document significant variation in technical maturity, community engagement, and sustainability practices; the 2017 Knight/Rita Allen successor to [11] explicitly documents scaling and sustainability challenges among civic-tech startups and non-profits. Schrock [15] frames civic tech as a "practice of technical pluralism," usefully distinguishing umbrella definitions from bucket definitions. Saldivar et al. [14] provided an empirical CSCW-journal investigation of civic technology for social innovation, the most comparable prior quantitative civic-tech study to ours. Knutas et al. [18] present an activity-theory case study of a grassroots civic-tech software group, the most recent peer-reviewed empirical civic-tech-software paper we are aware of. The existing literature remains predominantly qualitative or case-based; quantitative landscape-scale repository-mining studies of civic-tech specifically are rare. Our work begins to fill that gap.
...
A repository is included in the sample if it satisfies *all* of:
...
**Cross-project ecosystem.** Per contributor: number of repositories in our sample committed to. Bots filtered before computing the human cross-project distribution. We separately report (i) the fraction of contributors active in ≥ 2 of the 37 repositories and (ii) the fraction whose multiple-repo activity spans ≥ 2 distinct stewarding organisations — the latter is the sensitivity check for the umbrella-network selection bias (§6).
...
**Week-by-week concentration (novel metric).** The per-repository median elephant-week share is **96.6%** (IQR 94.9–100.0%, range 43.6–100.0%). Pooled across all repository-weeks weighted by activity, 83.3% of active weeks are elephant weeks.

### 4.3 Challenge 3 — Effort concentration exceeds activity concentration

*This is a novel finding. We are not aware of prior literature reporting a paired within-repository comparison of effort-weighted vs commit-count Gini across an OSS-domain sample.*
...
**This is survivor bias, not health.** Our sample includes only projects with ≥ 1 commit in the preceding 12 months. Civic-tech projects that died are excluded. Splitting the 37 repositories at the median age (6.3 years) confirms mature projects (n = 19) differ substantially from young (n = 18) on size proxies (Mann–Whitney U on `num_developers`: p = 0.004, Cliff's δ = +0.61 large; on `total_commits`: p = 0.009, δ = +0.56 large; on `bus_factor_no_bots`: p = 0.036, δ = +0.46 medium–large), but `stale_issue_ratio` shows only a small effect (δ = +0.24). The substantive observation that survives selection is narrower but real: *the surviving civic-tech projects in our sample show no evidence of activity decline*, consistent with a model in which projects either die outright or stabilise into a long-lived activity regime.
...
**Contributor counts and denominators.** We work from the contributor-lifecycle table (per-repository attribution), which yields **2,055 unique human contributor logins** across the 37-repository sample after bot filtering. (An alternative sample-wide deduplication, `cross_project_overlap.csv`, uses a stricter GraphQL-author key and yields 511 unique logins of which 498 are human; for the sensitivity check below we need per-repository attribution, so the lifecycle table is the primary source throughout this subsection.) Of the 2,055 unique humans, **112 (5.5%) are active in ≥ 2 repositories in our sample**. The most cross-project human, `kilemensi`, contributes to 8 repositories in our sample all under Code for Africa; the most cross-project bot, `dependabot[bot]`, contributes to 23 repositories.

**Sensitivity check (sampling-frame bias).** A reviewer's concern: our sampling frame seeded heavily from umbrella networks (Code for America, Code for Africa, MySociety, Democracy Club, OKF, Code for Japan), so an "umbrella-shaped ecosystem" finding could be an artefact. We address by re-running the analysis at the **organisation** level: of the 112 multi-repo humans, only **10 (8.9%) span ≥ 2 distinct stewarding organisations**. The remaining 102 (91.1%) stay within a single organisation — most often Code for Africa, secondarily Code for America. Translating to the all-contributor denominator: of the 2,055 unique humans, ≈ 5.5% are cross-project across the 37 repositories and ≈ 0.5% are cross-organisational. The umbrella-bounded pattern is not a sampling artefact: it survives at the organisation level, where umbrella over-representation cannot inflate the cross-project rate.

![Figure 5](figures/fig_cross_project_v2.png)

**Figure 5.** Top-15 human cross-project contributors (by cross-repository count). Teal: stays within one organisation. Amber: spans ≥ 2 organisations. 91.1% of cross-project humans are teal.
...
We presented emerging results from a multi-dimensional landscape analysis of 37 civic-tech repositories. Six sustainability challenges emerge: drive-by contribution dominates (52% single-commit), cores are highly concentrated (median bus factor 2; 46% at bus factor 1; per-repo median 96.6% elephant weeks), effort concentration systematically exceeds activity concentration (Wilcoxon p = 4.8 × 10⁻⁵, r = +0.81), stale-issue backlogs are pervasive (median ratio 0.98), release discipline is largely absent (54%), and the cross-project contributor ecosystem is thin (5.5% of unique humans) and umbrella-bounded (only 0.5% cross-organisational). A counter-pattern — surviving older projects intensify rather than decay — characterises survivors only.

Work in progress along four axes:

- **(L1) Longitudinal tracking** — quarterly recrawls of the 37 repositories over 24 months, with change-over-time analyses on the six challenges and event-study designs around governance changes.
...
- **(L3) Failed-projects comparison group** — a parallel sample of civic-tech repositories that have ceased activity in the last five years, supporting causal claims about which sustainability characteristics distinguish survivors from non-survivors.
- **(L4) Matched commercial-OSS comparison sample** — a same-size sample of small-team commercial-OSS projects (matched on language, age, team size) crawled with the same toolchain, to test whether the six civic-tech challenges differ in magnitude from comparable non-civic-tech projects — the addressable form of the "how does civic-tech compare to general OSS?" question.
...
[14] Saldivar, J., Parra, C., Alcaraz, M., Arteta, R., & Cernuzzi, L. (2019). Civic Technology for Social Innovation. *Computer Supported Cooperative Work (CSCW)*, 28(1–2), 169–207. DOI: 10.1007/s10606-018-9311-7

[15] Schrock, A.R. (2019). What is Civic Tech? Defining a Practice of Technical Pluralism. In P. Cardullo, C. Di Feliciantonio, & R. Kitchin (Eds.), *The Right to the Smart City* (pp. 125–134). Emerald Publishing. DOI: 10.1108/978-1-78769-139-120191009

[16] Knight Foundation & Rita Allen Foundation. (2017). *Scaling Civic Tech: Paths to a Sustainable Future*. <https://knightfoundation.org/reports/scaling-civic-tech/>

[17] Lumbard, K., Germonprez, M., & Goggins, S.P. (2024). An empirical investigation of social comparison and open source community health. *Information Systems Journal*, 34(2), 499–532. DOI: 10.1111/isj.12485

[18] Knutas, A., Siemon, D., Tylosky, N., & Maccani, G. (2023). Contradicting Motivations in Civic Tech Software Development: Analysis of a Grassroots Project. In *Proc. IEEE/ACM 45th International Conference on Software Engineering: Software Engineering in Society (ICSE-SEIS)* (pp. 149–160). DOI: 10.1109/ICSE-SEIS58686.2023.00021
